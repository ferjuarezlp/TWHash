...
public class ChipsMultiAutoCompleteTextview extends MultiAutoCompleteTextView implements AdapterView.OnItemClickListener {
    ...
    private TextWatcher textWather = new TextWatcher() {
        ...
        if(count >=1){
            if(s.charAt(start) == ',' || s.charAt(start) == ' ')
               ...
               // getText().delete(getText().toString().lastIndexOf(" "), getText().toString().length());
            }
        }
    ...
    public void setChips(){
        if(getText().toString().contains(",") || getText().toString().contains(" ")) // check comman in string
        {
            SpannableStringBuilder ssb = new SpannableStringBuilder(getText());
            // split string wich comma
            String chips[] = null;
            if(getText().toString().contains(",")){
              chips= getText().toString().trim().split(",");
            }
